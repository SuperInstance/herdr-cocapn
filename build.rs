// build.rs — herdr-cocapn fork
// The full herdr build requires Zig 0.15.2+ for the vendored libghostty-vt.
// For development and documentation validation, set SKIP_VENDORED_BUILD=1.

use std::env;

fn main() {
    if env::var("SKIP_VENDORED_BUILD").is_ok() {
        println!("cargo:warning=SKIP_VENDORED_BUILD=1: vendored libghostty-vt build skipped");
        return;
    }

    // Original build logic (requires Zig 0.15.2+)
    full_build();
}
